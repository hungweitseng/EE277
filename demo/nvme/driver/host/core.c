		...
		blk_mq_req_flags_t flags)
{
	struct request *req;
	int ret;
//		printk("A sync command (%X) is issued to LBA: %X\n", (struct nvme_rw_command *)(cmd)->opcode, (struct nvme_rw_command *)(cmd)->slba);
		printk("A sync command is received\n");
		...
		u32 meta_seed, u32 *result, unsigned timeout)
{
	...
	if (ubuffer && bufflen) {
		...
		printk("A command is issued to hardware\n");
	blk_execute_rq(req->q, disk, req, 0);
	if (nvme_req(req)->flags & NVME_REQ_CANCELLED)
		ret = -EINTR;
	else
		ret = nvme_req(req)->status;
		printk("A command returned\n");
	if (result)
...
//		c.rw.rsvd2 = io.rsvd;
	...
	case nvme_cmd_write:
	case nvme_cmd_read:
		printk("A command (%X) is issued to LBA: %X\n", io.opcode, io.slba);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	struct nvme_ns *ns;
	int srcu_idx, ret;
	printk("A command is received at IOCTL.\n");
